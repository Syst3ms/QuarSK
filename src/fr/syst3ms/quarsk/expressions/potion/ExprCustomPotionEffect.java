...
public class ExprCustomPotionEffect extends SimpleExpression<PotionEffect> {
    ...
    static {
        Registration.newExpression("A custom potion effect", ExprCustomPotionEffect.class, PotionEffect.class, ExpressionType.COMBINED, "[[potion] effect [(with|by)]] %potioneffecttype% for %timespan% with [a] [tier [of]] %number% [particles %-boolean%[ with ambient [effect] %-boolean%[ and [particle] colo[u]r[ed] %-color%]]]]]");
    }
