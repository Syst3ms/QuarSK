...
public class Quarsk extends JavaPlugin {
    ...
    private void normalRegister() {
         ...
         */
        //Potions
        Classes.registerClass(new ClassInfo<>(PotionEffect.class, "potioneffect").name("potioneffect").description("A getter for potion effects").user("potion ?effect").parser(new Parser<PotionEffect>() {
            @Override
            @Nullable
        ...
        //Banners
        Classes.registerClass(new ClassInfo<>(Pattern.class, "bannerlayer").name("banner layer").description("A getter for banner layers").user("banner ?(layer|pattern)").parser(new Parser<Pattern>() {
            ...
            public String toString(@NotNull Pattern pattern, int i) {
                return "layer with pattern " + EnumType.toString(pattern.getPattern().name()) + " and color " + pattern.getColor().toString().toLowerCase();
            }
        ...
        // Event values
        EventValues.registerEventValue(PotionSplashEvent.class, Projectile.class, new Getter<Projectile, PotionSplashEvent>() {
            @Override
            public Projectile get(PotionSplashEvent potionSplashEvent) {
                return potionSplashEvent.getEntity();
            }
        }, 0);
        EventValues.registerEventValue(LingeringPotionSplashEvent.class, Projectile.class, new Getter<Projectile, LingeringPotionSplashEvent>() {
            @Override
            public Projectile get(LingeringPotionSplashEvent lingeringPotionSplashEvent) {
                return lingeringPotionSplashEvent.getEntity();
            }
        }, 0);
        EventValues.registerEventValue(LingeringPotionSplashEvent.class, Entity.class, new Getter<Entity, LingeringPotionSplashEvent>() {
            @Override
